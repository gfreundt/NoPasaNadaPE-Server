from updates import xxxget_recipients


def get_records_alertas(db_cursor):

    # update list of recipients
    xxxget_recipients.actualizar_alerta(db_cursor)
...
def get_records_boletines(db_cursor):

    # update list of recipients
    xxxget_recipients.need_alert(db_cursor)
    xxxget_recipients.need_message(db_cursor)
...
def get_records_soats(db_cursor, thresh, HLA, HLA2):
    ...
    db_cursor.execute(
        f"""SELECT Placa FROM _necesitan_mensajes_placas
            ...
            UNION
            SELECT Placa FROM _actualizar_alertas
