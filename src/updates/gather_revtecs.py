...
def gather(
    ...
    while True:
        ...
        try:
            ...
            placa = record_item
            logger.info(f"Revisión Técnica: Obtenido de cola: {placa}")
        except Empty:
            logger.info("Revisión Técnica: Fin de cola.")
        ...
        try:
            ...
            logger.info(f"Revisión Técnica ({placa}): Iniciando scraper")
            ...
            if isinstance(scraper_response, str):
                logger.warning(f"Revisión Técnica ({placa}): {scraper_response}")
                dash.log(
                    card=card,
                    status=2,
                ...
                if "@" in scraper_response:
                    logger.info(
                        f"Revisión Técnica ({placa}): Reiniciando en 10 segundos."
                    )
                    dash.log(
                        card=card,
            ...
            if not scraper_response:
                logger.info(f"Revisión Técnica ({placa}): Sin datos encontrados.")
                with lock:
            ...
            with lock:
                ...
                logger.info(
                    f"Revisión Técnica ({placa}): Datos obtenidos: {pformat(respuesta)}."
                )
            ...
            logger.error(f"Revisión Técnica ({placa}): Exception: {str(e)}")
            # devolver registro a la cola para que otro thread lo complete
            if record_item is not None:
                queue_update_data.put(record_item)
    ...
    logger.info(f"Revisión Técnica ({placa}): Cerrando webdriver.")
    webdriver.quit()
